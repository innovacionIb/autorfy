...
return{
	get_all_obra: function(req, res){
		console.log("getting all obra from database: ");
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    // queryAllobra - requires no arguments , ex: args: [''],
		    const request = {
	...
	add_user: function(req, res){
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
			...
			return fabric_ca_client.register({enrollmentID: req.params.User, affiliation: 'org1.department1'}, admin_user);
		}).then((secret) => {
			// next we need to enroll the user with CA server
			console.log('Successfully registered user - secret:'+ secret);

			return fabric_ca_client.enroll({enrollmentID: req.params.User, enrollmentSecret: secret});
		}).then((enrollment) => {
	...
	add_obra: function(req, res){
		console.log("submit recording of a obra catch: ");
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    // recordObra - requires 5 args, ID, vessel, location, timestamp,holder - ex: args: ['10', 'Hound', '-12.021, 28.012', '1504054225', 'Hansel'], 
		    // send proposal to endorser
		    const request = {
		        //targets : --- letting this default to the peers assigned to the channel
		        chaincodeId: 'autorfy-app',
		        fcn: 'recordObra',
		        args: [key,description, location, usuario, fechacreacion,imagen,timestamp,tag],
		        chainId: 'mychannel',
		        txId: tx_id
		    };
	...
	get_obra: function(req, res){
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    // queryObra - requires 1 argument, ex: args: ['4'],
		    const request = {
		        chaincodeId: 'autorfy-app',
		        txId: tx_id,
		        fcn: 'queryObra',
		    ...
		    if (query_responses && query_responses.length == 1) {
		        if (query_responses[0] instanceof Error) {
		            console.error("error from query = ", query_responses[0]);
		            res.send("Could not locate obra")
